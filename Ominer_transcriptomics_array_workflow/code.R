...
"A2Rplot.hclust" <- function(
  ...
  if(show.labels){
    ...
    par(mar=c(0,0,0,4))
    par(srt=90)
    obs.labels <- toupper(substr(x$labels[x$order],1,100))
    if(is.null(members)) {
      plot(0,type="n",xlim=c(0.5,n.indiv+1), ylim=c(0,1), xaxs="i", axes=FALSE, xlab="",ylab="") 
      text(1:n.indiv            , 0, obs.labels, pos=4, col=col.down[groups.o], cex=0.8)
    }
    else{
      plot(0,type="n",xlim=c(0.5,sum(members)+.5), ylim=c(0,1), xaxs="i", axes=FALSE, xlab="",ylab="") 
      xo <-   members[x$order]
      text(cumsum(xo)-xo/2, 0, obs.labels, pos=4, col=col.down[groups.o], cex=0.7)
    }
    par(srt=0)
    if(boxes){
      box()
  ...
  if(!missing(fact.sup)){
    ...
    par(las=1)
    axis(2, (1:n.levels)-.5,levels(quali), tick=TRUE, cex.axis=1)
# text(.5,.5,"Study")
    if(boxes){
      box()
    }
    ...
    par(mar=c(1,0,0,0))
    plot(0,type="n",xlim=c(0,1), ylim=c(0,1), xaxs="i", axes=FALSE, xlab="",ylab="") 
#    text(.2,.2,deparse(substitute(fact.sup)))
