##########################################################################################
#
#File name: heatmap_create_TRIMMED_MIRNA.R
#
...
x <- read.table(paste("ominer_results/",project,"/","DifferentialExpression","/",ci,"_Filtered.txt",sep=""), header = T, sep = "\t",as.is=T) 
   ...
   esetSel = trim_A.data[v, ]  ###problem here extracting the relevant rows
    ...
    png(paste(readdir, "/DifferentialExpression/",ci,".heatmap_TRIMMED.png",sep=""))
    res=900
    ...
    pdf(paste(readdir, "/DifferentialExpression/",ci,".heatmap_TRIMMED.pdf",sep=""))
    res=900
